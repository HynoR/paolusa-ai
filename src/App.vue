<template>
  <div class="container">
    <el-container>
      <el-header>
        <h5>ChatAPI Beta V2@build-07021051 | <el-button @click="refreshUserInfo">刷新数据</el-button></h5>
      ...
      <el-main>
        <el-card>
          <el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleClick">
            ...
            <el-menu-item index="7">工具-文生图</el-menu-item>
...
const getInfo = async () => {
 if (!token.value) {
  ...
  try {
    const response = await axios.get('https://lab.nloli.xyz/tako_web/gpt_info?token=' + token.value);
